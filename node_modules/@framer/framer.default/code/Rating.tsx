...
export function Rating(props) {
    ...
    const handleChange = ratingVal => {
        ratingSpring.set(ratingVal)
        if (onChange) onChange(ratingVal)
        if (onFill && ratingVal === amount) onFill()
        if (onEmpty && ratingVal === 0) onEmpty()
    }
    ...
    useOnEnter(() => {
        if (onLoadReset) ratingSpring.set(rating)
    })

    return (
        <div style={parentStyle}>
            <div onClick={() => handleChange(0)} style={{ height: size, width: 10 }} />
            <motion.div
                ref={parent}
                onTap={handleTap}
...
addPropertyControls(Rating, {
    ...
    onLoadReset: {
        ...
        type: ControlType.Boolean,
        enabledTitle: "Reset",
        disabledTitle: "Persist",
    },
    onChange: { type: ControlType.EventHandler },
    onEmpty: { type: ControlType.EventHandler },
    onFill: { type: ControlType.EventHandler },
    ...defaultEvents,
})
